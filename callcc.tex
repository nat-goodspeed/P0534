...
output.\\
The expression \cpp{c1()} at (b) resumes the original
...
The call to \cpp{c2()} at (d) (the \emph{updated} \cpp{c2})
resumes function \cpp{foo}, returning from the \op call at (b) and executing
the assignment at (e). Here, too, we replace the \cont instance \cpp{c1}
...
In order to transfer data, \call as well as \op accept arguments, that are
...
transferred via \op at (b). The execution control returns and \cpp{c2}

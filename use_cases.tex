...
the work the kernel thread chooses to do at this moment. When context
...
\item The fiber manager keeps fiber objects in separate containers: those
  still waiting for something versus those that are ready
  to resume.
...
\item[Connecting output iterator to input iterator] In February 2017, a
  ...
  takes an output iterator, be engaged lazily? Can consumer code request
  one item at a time?\\
...
\begin{equation*} \cfrac{2^{32} - (\text{size of all code}) - (\text{size of all other data})}{(\text{stack size})} \end{equation*} \\

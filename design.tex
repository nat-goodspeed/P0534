...
\item Call \termthen with a valid continuation. This throws a \unwindex
...
throw \unwindex ``by hand,'' which is what \termthen does internally.]}
\item
Call \cpp{std::continuation::resume\_with(std::terminate\_context\_then)}.
This is what \dtor does. Since \termthen accepts a \cont, and
...
If a \resumewith function throws an exception, it is good practice to bind
into the exception object the continuation passed into the \resumewith
function so that a suitable \cpp{catch} clause in the \entryfn can return that
continuation.

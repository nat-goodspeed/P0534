...
retrieve the data.
\cppf{passing_single}
The \cpp{callcc()} call at (a) enters the lambda and passes 1 into the
new context. The value is retrieved as \cpp{j}, as shown by (b). The expression
\cpp{caller(j+1)} at (c) resumes the original context (represented
within the lambda by \cpp{caller}) and transfers back an integer of \cpp{j+1}.
The assignment at (d) sets \cpp{i} to \cpp{j+1}.\\
The call to \cpp{lambda()} at (e) (note that no data is passed) resumes the
lambda, returning from the \cpp{caller(j+1)} call at (c). Here, too,
we replace the \cont instance \cpp{caller} invalidated by the \resume call at (c)
with the new instance returned by that same \resume call.\\
Finally the lambda returns (the updated) \cpp{caller} at (f), terminating its
context.\\
...
\cppf{simple}
The \cpp{callcc()} call at (a) enters the lambda. The \cont\ \cpp{caller} at (b)
represents the execution context of \main. Returning \cpp{caller} at (c) resumes the
...
At this point the main context calls \cpp{f(invoke\_ontop\_arg, g);}
...
The \cpp{f\_ct(invoke\_ontop\_arg, g, data+1)} call at (i) passes control
to \cpp{g()} on the continuation of \cpp{f()}.\\
The \cpp{return} statement at (j) causes the \resume call at (g) to return,
executing the assignment at (k). The \cpp{int} returned by \cpp{g()} is
accessed at (l).\\
